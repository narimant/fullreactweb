...
export default function CommentsTextArea({comments,submitComment}) {
  ...
  return (
    <div className="comments">
      ...
      {comments.length ==0 ? (
        <div className="alert alert-warning  text-center">هنوز کامنتی ارسال نشده است</div>
      ) : (
        <>
         {
              ...
              <div className="comments__question-header">
                <div className="comments__question-header-right">
                  ...
                  <span className="comments__question-status comment-status">
                    {comment.creator.role=='ADMIN'?  (<span> مدیر  </span>) : (<span> کاربر </span>)}
                  </span>
      ...
      <div className="comments__respond">
        ...
        <div className="comments__respond-content">
          ...
          <textarea className="comments__score-input-respond" onChange={changeHandler} value="" >
            {newCommentBody}
          </textarea>
        </div>
        <button type="submit" className="comments__respond-btn" onClick={()=>submitComment(newCommentBody)}>
          ...
          <div className="alert alert-warning text-center mt-4 mb-4">
              برای ارسال کامنت ابتدا باید <Link to='/login'>وارد حساب کاربری</Link> خود شده باشید
          </div>
         )}
